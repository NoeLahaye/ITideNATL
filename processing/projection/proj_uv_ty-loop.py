...
if __name__ == "__main__":
    ...
    cluster = LocalCluster(n_workers=8, dashboard_address=":8789", interface="ib0", 
            local_directory=scratch) #n_workers=24, threads_per_worker=1, memory_limit=8e6,silence_logs=50
    client=Client()
    ...
    if len(sys.argv)>1:
        ...
        var = "u" # choose "u" or "v"
    restart = 0 # continue previously stopped job (y segments). False or None starts from beginning creating a new zarr store 
    ...
    print("debugging", restart, len(ind_y)-1, ind_y)
